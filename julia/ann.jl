...
# ╔═╡ 90c8df69-590c-4d97-a0cb-792df7e88fb5
begin
	using CSV
	using StatsBase
	using DataFrames
...
# ╔═╡ bca419fe-676d-11ec-2666-e79fde06eb91
html"""
...
# ╔═╡ 4a0182c3-bc6d-42f2-ba3c-107478e8351d
df = CSV.read("../dataset/coins.csv",DataFrame);

# ╔═╡ 080d1df6-43ae-422c-9c6b-26d4c3c5a0e0
begin
	data = Array(df);
	Ndata = size(data)[1];
end

# ╔═╡ d2e818af-b957-460a-bb70-d3240db69b7b
begin
	train_idx = sample(1:Ndata, 160, replace=false);
	test_idx = filter(x->!(x in train_idx), 1:Ndata);
	X_train, y_train = data[train_idx,1:2], data[train_idx,end] .- 1;
	X_test, y_test = data[test_idx,1:2], data[test_idx,end] .- 1; 
end

# ╔═╡ 6340bbf1-3bd1-4e67-bd31-d0b5d52d5958
dataloaded = Flux.Data.DataLoader((X_train', y_train), batchsize=5,shuffle=true);

# ╔═╡ 91dbcc95-b99e-4a7c-be1c-e49b02dae934
...
# ╔═╡ 5e223254-ee87-455e-8a96-6747e0b5b91a
model=NeuralNetwork()

# ╔═╡ 5ee0895b-0d00-4d33-bf97-cfcad4f1f42d
opt = Descent(0.003)

# ╔═╡ d0609a9a-ee18-4e10-a1ee-f1665861f77b
loss(x, y) = sum(Flux.Losses.binarycrossentropy(model(x), y))

# ╔═╡ fcbbb7b1-1f1b-45be-ac12-5cc0dc509ab3
begin
	ps = Flux.params(model)
	epochs = 300
	for i in 1:epochs
	    Flux.train!(loss, ps, dataloaded, opt)
	end
end

# ╔═╡ e812619d-2997-4a7b-a65e-21128edbe426
loss(X_test',y_test)

# ╔═╡ 2cebe392-b1e1-450e-9d2f-696c47a1c28c
model(X_test') .> 0.5
...
git-tree-sha1 = "ca354881c303a7bef03d8710015ae345b725c166"
